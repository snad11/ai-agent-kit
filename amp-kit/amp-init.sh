#!/usr/bin/env bash
# amp-init.sh — install the Amp kit into a target project.
#
...
#
# Usage:
#     bash amp-init.sh [target-project-path]
#     bash amp-init.sh .
#
# Emits:
#     .agents/rules.md            rules (universal baseline + detected stack modules)
...
if [ "$TARGET_INPUT" = "--help" ] || [ "$TARGET_INPUT" = "-h" ]; then
    cat <<EOF
${BOLD}amp-init.sh${NC} — install the Amp kit into a target project

${BOLD}Usage:${NC}
    bash amp-init.sh [target-path]
...
${BOLD}Examples:${NC}
    bash $KIT_DIR/amp-init.sh
    bash $KIT_DIR/amp-init.sh /path/to/project

${BOLD}After running:${NC}
    Reference @$KIT_DIR/BOOTSTRAP.md in Amp to finish the
...
# Directories
# ============================================================
mkdir -p "$(dirname "$TARGET/.agents/rules.md")"
...
RULES_OUT="$TARGET/.agents/rules.md"
...
cat <<EOF
${GREEN}${BOLD}OK amp-init complete${NC}
...
${BOLD}Note:${NC} Thin AGENTS.md consumer; no per-tool directory
