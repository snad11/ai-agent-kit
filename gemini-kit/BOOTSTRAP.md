# BOOTSTRAP — set up a project with the Gemini CLI kit

Reference this file in Gemini CLI from the project you want to bootstrap:

```
@$KIT/gemini-kit/BOOTSTRAP.md please bootstrap this project
```
...
You are bootstrapping a project with the Gemini CLI kit. Multi-phase. Do not skip
...
### Phase 3 — Audit
For each detected repo, run the audit in `.gemini/templates/audit-prompt.md`
over every source file, line by line. Use background agents where the tool
supports them. Write findings under `audits/`.

### Phase 4 — Rules
Review `.gemini/rules.md`. Append project-specific rules discovered in Phase 3, using
the existing ID scheme and the Why/Detect/Fix template.

### Phase 5 — Memory
Author the project memory files, then re-run the memory clone so `GEMINI.md`
carries them verbatim:

```bash
bash .gemini/scripts/clone-memory.sh GEMINI.md <memory-dir>
```

### Phase 6 — Hooks and CI
```bash
bash .gemini/scripts/install-hooks.sh all
bash .gemini/scripts/install-workflows.sh all
```
...
- **Do not modify source code.** Only `.gemini/`, `GEMINI.md`, `audits/`
