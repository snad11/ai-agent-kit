#!/usr/bin/env bash
# gemini-init.sh — install the Gemini CLI kit into a target project.
#
...
#
# Usage:
#     bash gemini-init.sh [target-project-path]
#     bash gemini-init.sh .
#
# Emits:
#     .gemini/rules.md            rules (universal baseline + detected stack modules)
#     GEMINI.md           anchor file the agent reads, with memory cloned in
#     .gemini/commands/  (toml commands generated from the shared skills)
#
...
if [ "$TARGET_INPUT" = "--help" ] || [ "$TARGET_INPUT" = "-h" ]; then
    cat <<EOF
${BOLD}gemini-init.sh${NC} — install the Gemini CLI kit into a target project

${BOLD}Usage:${NC}
    bash gemini-init.sh [target-path]
...
${BOLD}Examples:${NC}
    bash $KIT_DIR/gemini-init.sh
    bash $KIT_DIR/gemini-init.sh /path/to/project

${BOLD}After running:${NC}
    Reference @$KIT_DIR/BOOTSTRAP.md in Gemini CLI to finish the
...
# Directories
# ============================================================
mkdir -p "$(dirname "$TARGET/.gemini/rules.md")"
mkdir -p "$(dirname "$TARGET/GEMINI.md")"
...
SCRIPTS_DIR="$TARGET/.gemini/scripts"
mkdir -p "$SCRIPTS_DIR"
...
mkdir -p "$TARGET/.gemini/workflows"
subst_copy "$KIT_DIR/workflows/x-check.yml" "$TARGET/.gemini/workflows/x-check.yml"
printf '  %sok%s workflows/x-check.yml\n' "$GREEN" "$NC"

mkdir -p "$TARGET/.gemini/templates"
subst_copy "$KIT_DIR/templates/audit-prompt.md" "$TARGET/.gemini/templates/audit-prompt.md"
...
RULES_OUT="$TARGET/.gemini/rules.md"
...
for skill_dir in "$KIT_DIR/skills"/*/; do
    ...
    body="$(sed '1{/^---$/!q;}; 1,/^---$/d' "$tmp")"
    {
        ...
        printf 'prompt = """\n%s\n"""\n' "$body"
    } > "$TARGET/.gemini/commands/${name}.toml"
    rm -f "$tmp"
    printf '  %sok%s .gemini/commands/%s.toml\n' "$GREEN" "$NC" "$name"
done
printf '\n'
...
# Anchor + memory clone
# ============================================================
printf '%sGenerating GEMINI.md...%s\n' "$BOLD" "$NC"
subst_copy "$KIT_DIR/templates/GEMINI.md.template" "$TARGET/GEMINI.md"
if ls "$MEMORY_ROOT"/*.md >/dev/null 2>&1; then
    bash "$SCRIPTS_DIR/clone-memory.sh" "$TARGET/GEMINI.md" "$MEMORY_ROOT" >/dev/null || \
        printf '  %swarn%s memory clone skipped\n' "$YELLOW" "$NC"
fi
printf '  %sok%s GEMINI.md (memory cloned)\n\n' "$GREEN" "$NC"
...
cat <<EOF
${GREEN}${BOLD}OK gemini-init complete${NC}

${BOLD}Installed at:${NC}
  ${TARGET}/GEMINI.md
  ${TARGET}/.gemini/rules.md
...
${BOLD}Note:${NC} Custom commands are TOML with prompt/description; reload with /commands reload

${BOLD}Next steps:${NC}
  1. Finish the bootstrap conversationally — reference:
     ${BLUE}@${KIT_DIR}/BOOTSTRAP.md${NC}
  2. Install git hooks:  bash ${TARGET}/.gemini/scripts/install-hooks.sh all
  3. Stage CI workflow:  bash ${TARGET}/.gemini/scripts/install-workflows.sh all
EOF
