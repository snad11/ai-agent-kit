#!/usr/bin/env bash
# cursor-init.sh — install the Cursor kit into a target project.
#
...
#
# Usage:
#     bash cursor-init.sh [target-project-path]
#     bash cursor-init.sh .
#
# Emits:
#     .cursor/rules/000-project-rules.mdc            rules (universal baseline + detected stack modules)
#     AGENTS.md           anchor file the agent reads, with memory cloned in
#     .cursor/commands/  (mdc commands generated from the shared skills)
#
...
if [ "$TARGET_INPUT" = "--help" ] || [ "$TARGET_INPUT" = "-h" ]; then
    cat <<EOF
${BOLD}cursor-init.sh${NC} — install the Cursor kit into a target project

${BOLD}Usage:${NC}
    bash cursor-init.sh [target-path]
...
${BOLD}Examples:${NC}
    bash $KIT_DIR/cursor-init.sh
    bash $KIT_DIR/cursor-init.sh /path/to/project

${BOLD}After running:${NC}
    Reference @$KIT_DIR/BOOTSTRAP.md in Cursor to finish the
...
# Directories
# ============================================================
mkdir -p "$(dirname "$TARGET/.cursor/rules/000-project-rules.mdc")"
...
SCRIPTS_DIR="$TARGET/.cursor/scripts"
mkdir -p "$SCRIPTS_DIR"
...
mkdir -p "$TARGET/.cursor/workflows"
subst_copy "$KIT_DIR/workflows/x-check.yml" "$TARGET/.cursor/workflows/x-check.yml"
printf '  %sok%s workflows/x-check.yml\n' "$GREEN" "$NC"

mkdir -p "$TARGET/.cursor/templates"
subst_copy "$KIT_DIR/templates/audit-prompt.md" "$TARGET/.cursor/templates/audit-prompt.md"
...
RULES_OUT="$TARGET/.cursor/rules/000-project-rules.mdc"
...
printf '  %sok%s .cursor/rules/000-project-rules.mdc\n\n' "$GREEN" "$NC"
...
for skill_dir in "$KIT_DIR/skills"/*/; do
    ...
    subst_copy "$skill_dir/SKILL.md" "$TARGET/.cursor/commands/${name}.mdc"
    printf '  %sok%s .cursor/commands/%s.mdc\n' "$GREEN" "$NC" "$name"
done
printf '\n'
...
cat <<EOF
${GREEN}${BOLD}OK cursor-init complete${NC}

${BOLD}Installed at:${NC}
  ${TARGET}/AGENTS.md
  ${TARGET}/.cursor/rules/000-project-rules.mdc
...
${BOLD}Note:${NC} Reads AGENTS.md natively; .mdc adds per-glob scoping. Legacy .cursorrules is ignored by Agent mode

${BOLD}Next steps:${NC}
  1. Finish the bootstrap conversationally — reference:
     ${BLUE}@${KIT_DIR}/BOOTSTRAP.md${NC}
  2. Install git hooks:  bash ${TARGET}/.cursor/scripts/install-hooks.sh all
  3. Stage CI workflow:  bash ${TARGET}/.cursor/scripts/install-workflows.sh all
EOF
