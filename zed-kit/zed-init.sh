#!/usr/bin/env bash
# zed-init.sh — install the Zed kit into a target project.
#
...
#
# Usage:
#     bash zed-init.sh [target-project-path]
#     bash zed-init.sh .
#
# Emits:
#     .zed/rules.md            rules (universal baseline + detected stack modules)
#     .rules           anchor file the agent reads, with memory cloned in
...
if [ "$TARGET_INPUT" = "--help" ] || [ "$TARGET_INPUT" = "-h" ]; then
    cat <<EOF
${BOLD}zed-init.sh${NC} — install the Zed kit into a target project

${BOLD}Usage:${NC}
    bash zed-init.sh [target-path]
...
${BOLD}Examples:${NC}
    bash $KIT_DIR/zed-init.sh
    bash $KIT_DIR/zed-init.sh /path/to/project

${BOLD}After running:${NC}
    Reference @$KIT_DIR/BOOTSTRAP.md in Zed to finish the
...
# Directories
# ============================================================
mkdir -p "$(dirname "$TARGET/.zed/rules.md")"
mkdir -p "$(dirname "$TARGET/.rules")"
mkdir -p "$TARGET/.zed"
...
SCRIPTS_DIR="$TARGET/.zed/scripts"
mkdir -p "$SCRIPTS_DIR"
...
mkdir -p "$TARGET/.zed/workflows"
subst_copy "$KIT_DIR/workflows/x-check.yml" "$TARGET/.zed/workflows/x-check.yml"
printf '  %sok%s workflows/x-check.yml\n' "$GREEN" "$NC"

mkdir -p "$TARGET/.zed/templates"
subst_copy "$KIT_DIR/templates/audit-prompt.md" "$TARGET/.zed/templates/audit-prompt.md"
...
# Anchor + memory clone
# ============================================================
printf '%sGenerating .rules...%s\n' "$BOLD" "$NC"
subst_copy "$KIT_DIR/templates/AGENTS.md.template" "$TARGET/.rules"
if ls "$MEMORY_ROOT"/*.md >/dev/null 2>&1; then
    bash "$SCRIPTS_DIR/clone-memory.sh" "$TARGET/.rules" "$MEMORY_ROOT" >/dev/null || \
        printf '  %swarn%s memory clone skipped\n' "$YELLOW" "$NC"
fi
printf '  %sok%s .rules (memory cloned)\n\n' "$GREEN" "$NC"
printf '%sWriting AGENTS.md alongside .rules...%s\n' "$BOLD" "$NC"
cp "$TARGET/.rules" "$TARGET/AGENTS.md"
...
cat <<EOF
${GREEN}${BOLD}OK zed-init complete${NC}
...
${BOLD}Note:${NC} .rules outranks AGENTS.md in Zed's priority list, so this kit writes both and keeps them consistent

${BOLD}Next steps:${NC}
  1. Finish the bootstrap conversationally — reference:
     ${BLUE}@${KIT_DIR}/BOOTSTRAP.md${NC}
  2. Install git hooks:  bash ${TARGET}/.zed/scripts/install-hooks.sh all
  3. Stage CI workflow:  bash ${TARGET}/.zed/scripts/install-workflows.sh all
EOF
