#!/usr/bin/env bash
# Template substitution for cline-init.sh.
#
...
# Path tokens are fixed for this kit because it only ever emits Cline layout.
...
subst_copy() {
    ...
    sed -e "s|{{PROJECT_ROOT}}|${TARGET}|g" \
        ...
        -e "s|{{AGENT_TARGET}}|cline|g" \
        -e "s|{{ANCHOR_FILE}}|AGENTS.md|g" \
        -e "s|{{RULES_PATH}}|.clinerules/project-rules.md|g" \
        -e "s|{{SKILLS_DIR}}|.clinerules/skills|g" \
        ...
        -e "s|{{TEMPLATES_DIR}}|.clinerules/templates|g" \
        "$src" > "$dst"
}
