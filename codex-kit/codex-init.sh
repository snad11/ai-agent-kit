#!/usr/bin/env bash
# codex-init.sh — install the Codex CLI kit into a target project.
#
...
#
# Usage:
#     bash codex-init.sh [target-project-path]
#     bash codex-init.sh .
#
# Emits:
#     .codex/rules.md            rules (universal baseline + detected stack modules)
...
if [ "$TARGET_INPUT" = "--help" ] || [ "$TARGET_INPUT" = "-h" ]; then
    cat <<EOF
${BOLD}codex-init.sh${NC} — install the Codex CLI kit into a target project

${BOLD}Usage:${NC}
    bash codex-init.sh [target-path]
...
${BOLD}Examples:${NC}
    bash $KIT_DIR/codex-init.sh
    bash $KIT_DIR/codex-init.sh /path/to/project

${BOLD}After running:${NC}
    Reference @$KIT_DIR/BOOTSTRAP.md in Codex CLI to finish the
...
# Directories
# ============================================================
mkdir -p "$(dirname "$TARGET/.codex/rules.md")"
...
SCRIPTS_DIR="$TARGET/.codex/scripts"
mkdir -p "$SCRIPTS_DIR"
...
mkdir -p "$TARGET/.codex/workflows"
subst_copy "$KIT_DIR/workflows/x-check.yml" "$TARGET/.codex/workflows/x-check.yml"
printf '  %sok%s workflows/x-check.yml\n' "$GREEN" "$NC"

mkdir -p "$TARGET/.codex/templates"
subst_copy "$KIT_DIR/templates/audit-prompt.md" "$TARGET/.codex/templates/audit-prompt.md"
...
RULES_OUT="$TARGET/.codex/rules.md"
...
cat <<EOF
${GREEN}${BOLD}OK codex-init complete${NC}

${BOLD}Installed at:${NC}
  ${TARGET}/AGENTS.md
  ${TARGET}/.codex/rules.md
...
${BOLD}Note:${NC} AGENTS.md discovery is tunable via ~/.codex/config.toml (project_doc_max_bytes)

${BOLD}Next steps:${NC}
  1. Finish the bootstrap conversationally — reference:
     ${BLUE}@${KIT_DIR}/BOOTSTRAP.md${NC}
  2. Install git hooks:  bash ${TARGET}/.codex/scripts/install-hooks.sh all
  3. Stage CI workflow:  bash ${TARGET}/.codex/scripts/install-workflows.sh all
EOF
