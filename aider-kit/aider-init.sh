#!/usr/bin/env bash
# aider-init.sh — install the Aider kit into a target project.
#
...
#
# Usage:
#     bash aider-init.sh [target-project-path]
#     bash aider-init.sh .
#
# Emits:
#     CONVENTIONS.md            rules (universal baseline + detected stack modules)
#     CONVENTIONS.md           anchor file the agent reads, with memory cloned in
...
if [ "$TARGET_INPUT" = "--help" ] || [ "$TARGET_INPUT" = "-h" ]; then
    cat <<EOF
${BOLD}aider-init.sh${NC} — install the Aider kit into a target project

${BOLD}Usage:${NC}
    bash aider-init.sh [target-path]
...
${BOLD}Examples:${NC}
    bash $KIT_DIR/aider-init.sh
    bash $KIT_DIR/aider-init.sh /path/to/project

${BOLD}After running:${NC}
    Reference @$KIT_DIR/BOOTSTRAP.md in Aider to finish the
...
# Directories
# ============================================================
mkdir -p "$(dirname "$TARGET/CONVENTIONS.md")"
mkdir -p "$(dirname "$TARGET/CONVENTIONS.md")"
...
SCRIPTS_DIR="$TARGET/.agents/scripts"
mkdir -p "$SCRIPTS_DIR"
...
mkdir -p "$TARGET/.agents/templates"
...
RULES_OUT="$TARGET/CONVENTIONS.md"
...
# Anchor + memory clone
# ============================================================
printf '%sGenerating CONVENTIONS.md...%s\n' "$BOLD" "$NC"
subst_copy "$KIT_DIR/templates/CONVENTIONS.md.template" "$TARGET/CONVENTIONS.md"
if ls "$MEMORY_ROOT"/*.md >/dev/null 2>&1; then
    bash "$SCRIPTS_DIR/clone-memory.sh" "$TARGET/CONVENTIONS.md" "$MEMORY_ROOT" >/dev/null || \
        printf '  %swarn%s memory clone skipped\n' "$YELLOW" "$NC"
fi
printf '  %sok%s CONVENTIONS.md (memory cloned)\n\n' "$GREEN" "$NC"
...
cat <<EOF
${GREEN}${BOLD}OK aider-init complete${NC}
...
${BOLD}Note:${NC} No slash-command concept: the skills ship as documented workflows inside CONVENTIONS.md, not invokable commands

${BOLD}Next steps:${NC}
  1. Finish the bootstrap conversationally — reference:
     ${BLUE}@${KIT_DIR}/BOOTSTRAP.md${NC}
  2. Install git hooks:  bash ${TARGET}/.agents/scripts/install-hooks.sh all
  3. Stage CI workflow:  bash ${TARGET}/.agents/scripts/install-workflows.sh all
EOF
