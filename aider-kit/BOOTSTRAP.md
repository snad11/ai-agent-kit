# BOOTSTRAP — set up a project with the Aider kit

Reference this file in Aider from the project you want to bootstrap:

```
@$KIT/aider-kit/BOOTSTRAP.md please bootstrap this project
```
...
You are bootstrapping a project with the Aider kit. Multi-phase. Do not skip
...
### Phase 4 — Rules
Review `CONVENTIONS.md`. Append project-specific rules discovered in Phase 3, using
the existing ID scheme and the Why/Detect/Fix template.

### Phase 5 — Memory
Author the project memory files, then re-run the memory clone so `CONVENTIONS.md`
carries them verbatim:

```bash
bash .agents/scripts/clone-memory.sh CONVENTIONS.md <memory-dir>
```
...
- **Do not modify source code.** Only `.agents/`, `CONVENTIONS.md`, `audits/`
