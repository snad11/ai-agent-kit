#!/usr/bin/env bash
# Template substitution for aider-init.sh.
#
...
# Path tokens are fixed for this kit because it only ever emits Aider layout.
...
subst_copy() {
    ...
    sed -e "s|{{PROJECT_ROOT}}|${TARGET}|g" \
        ...
        -e "s|{{AGENT_TARGET}}|aider|g" \
        -e "s|{{ANCHOR_FILE}}|CONVENTIONS.md|g" \
        -e "s|{{RULES_PATH}}|CONVENTIONS.md|g" \
